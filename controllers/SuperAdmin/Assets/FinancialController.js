...
const validateFinancialInformation = [
  body("capitalization_price").isDecimal().withMessage("Capitalization Price must be a decimal number").notEmpty().withMessage("Capitalization Price is required"),
  body("end_of_life_date").isDate().withMessage("End of Life Date must be a valid date").notEmpty().withMessage("End of Life Date is required"),
  body("capitalization_date").isDate().withMessage("Capitalization Date must be a valid date").notEmpty().withMessage("Capitalization Date is required"),
  body("depreciation_percentage").isFloat().withMessage("Depreciation Percentage must be a valid number").notEmpty().withMessage("Depreciation Percentage is required"),
  body("accumulated_depreciation").isDecimal().withMessage("Accumulated Depreciation must be a decimal number").notEmpty().withMessage("Accumulated Depreciation is required"),
  body("scrap_value").isDecimal().withMessage("Scrap Value must be a decimal number").notEmpty().withMessage("Scrap Value is required"),
  body("income_tax_depreciation_percentage")
    .isFloat()
    .withMessage("Income Tax Depreciation Percentage must be a valid number")
    .notEmpty()
    .withMessage("Income Tax Depreciation Percentage is required"),
];
...
const CreateFinancial = async (req, res) => {
  try {
    ...
    const { capitalization_price, end_of_life_date, capitalization_date, depreciation_percentage, accumulated_depreciation, scrap_value, income_tax_depreciation_percentage } =
      req.body;
...
const validateFinancialInformationUpdate = [
  body("capitalization_price").isDecimal().withMessage("Capitalization Price must be a decimal number").optional(),
  body("end_of_life_date").isDate().withMessage("End of Life Date must be a valid date").optional(),
  body("capitalization_date").isDate().withMessage("Capitalization Date must be a valid date").optional(),
  body("depreciation_percentage").isFloat().withMessage("Depreciation Percentage must be a valid number").optional(),
  body("accumulated_depreciation").isDecimal().withMessage("Accumulated Depreciation must be a decimal number").optional(),
  body("scrap_value").isDecimal().withMessage("Scrap Value must be a decimal number").optional(),
  body("income_tax_depreciation_percentage").isFloat().withMessage("Income Tax Depreciation Percentage must be a valid number").optional(),
];
...
const DeleteFinancial = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      message: "Error deleting financial information",
      error: error.message,
    });
  }
};
