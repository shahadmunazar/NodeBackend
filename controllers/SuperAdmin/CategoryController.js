...
const CreateCategory = async (req, res) => {
  await Promise.all([
    body("name").notEmpty().withMessage("Category name is required").isLength({ max: 100 }).withMessage("Category name must be less than 100 characters").run(req),
    body("description").optional().isLength({ max: 255 }).withMessage("Description must be under 255 characters").run(req),
    body("status").optional().isBoolean().withMessage("Status must be true or false").run(req),
  ]);
